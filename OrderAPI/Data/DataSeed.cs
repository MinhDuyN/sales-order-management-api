...
namespace OrderAPI.Data
{
    public static class DataSeed
    {
        ...
        private static void SeedUsers(AppDbContext db)
        {
            ...
            var customerRole = db.Roles.First(r => r.RoleName == "Customer");
            ...
            db.Users.AddRange(
                new User
                {
                    Name = "Admin",
                    Email = "admin@erp.com",
                    PasswordHash = BCrypt.Net.BCrypt.HashPassword("admin@erp"),
                    RoleId = adminRole.Id,
                    IsActive = true,
                ...
                new User
                {
                    Name = "Staff",
                    Email = "staff@erp.com",
                    PasswordHash = BCrypt.Net.BCrypt.HashPassword("staff@erp"),
                    RoleId = staffRole.Id,
                    IsActive = true,
                    CreatedDate = DateTime.UtcNow
                },
                new User
                {
                    ...
                    PasswordHash = BCrypt.Net.BCrypt.HashPassword("customer@erp"),
        ...
        private static void SeedProducts(AppDbContext db)
        {
            ...
            var laptop = db.Categories.First(c => c.CategoryName == "Laptop");
            var phone = db.Categories.First(c => c.CategoryName == "Điện thoại");
            var accessory = db.Categories.First(c => c.CategoryName == "Phụ kiện");
            var monitor = db.Categories.First(c => c.CategoryName == "Màn hình");
        ...
        private static void SeedOrders(AppDbContext db)
        {
            ...
            var customerA = db.Users.First(u => u.Email == "customer@erp.com");
            var customerB = db.Users.First(u => u.Email == "customer2@erp.com");

            var iphone = db.Products.First(p => p.ProductName == "iPhone 15 Pro");
            var mouse = db.Products.First(p => p.ProductName == "Chuột Logitech MX Master 3");
            var macbook = db.Products.First(p => p.ProductName == "MacBook Air M2");
            var dell = db.Products.First(p => p.ProductName == "Dell XPS 13");
            var sony = db.Products.First(p => p.ProductName == "Tai nghe Sony WH-1000XM5");
            var keyboard = db.Products.First(p => p.ProductName == "Bàn phím Keychron K2");
            var xiaomi = db.Products.First(p => p.ProductName == "Xiaomi 14");
            var samsung = db.Products.First(p => p.ProductName == "Samsung Galaxy S24");

            // Order 1: Customer A — Shipped
            var order1 = new Order
            {
                UserId = customerA.Id,
                Status = "Shipped",
                TotalAmount = iphone.Price * 1 + mouse.Price * 2,
                CreatedDate = now.AddDays(-20),
                OrderItems = new List<OrderItem>
                {
                    new OrderItem { ProductId = iphone.Id, Quantity = 1, UnitPrice = iphone.Price, LineTotal = iphone.Price * 1 },
                    new OrderItem { ProductId = mouse.Id,  Quantity = 2, UnitPrice = mouse.Price,  LineTotal = mouse.Price  * 2 }
                }
            };

            // Order 2: Customer A — Confirmed
            var order2 = new Order
            {
                UserId = customerA.Id,
                Status = "Confirmed",
                TotalAmount = macbook.Price * 1,
                CreatedDate = now.AddDays(-10),
                OrderItems = new List<OrderItem>
                {
                    new OrderItem { ProductId = macbook.Id, Quantity = 1, UnitPrice = macbook.Price, LineTotal = macbook.Price * 1 }
                }
            };

            // Order 3: Customer B — Shipped
            var order3 = new Order
            {
                UserId = customerB.Id,
                Status = "Shipped",
                TotalAmount = dell.Price * 1 + sony.Price * 1 + keyboard.Price * 2,
                CreatedDate = now.AddDays(-15),
                OrderItems = new List<OrderItem>
                {
                    new OrderItem { ProductId = dell.Id,     Quantity = 1, UnitPrice = dell.Price,     LineTotal = dell.Price     * 1 },
                    new OrderItem { ProductId = sony.Id,     Quantity = 1, UnitPrice = sony.Price,     LineTotal = sony.Price     * 1 },
                    new OrderItem { ProductId = keyboard.Id, Quantity = 2, UnitPrice = keyboard.Price, LineTotal = keyboard.Price * 2 }
                }
            };

            // Order 4: Customer B — Pending
            var order4 = new Order
            {
                UserId = customerB.Id,
                Status = "Pending",
                TotalAmount = xiaomi.Price * 1,
                CreatedDate = now.AddDays(-2),
                OrderItems = new List<OrderItem>
                {
                    new OrderItem { ProductId = xiaomi.Id, Quantity = 1, UnitPrice = xiaomi.Price, LineTotal = xiaomi.Price * 1 }
                }
            };

            // Order 5: Customer A — Cancelled
            var order5 = new Order
            {
                UserId = customerA.Id,
                Status = "Cancelled",
                TotalAmount = samsung.Price * 1,
                CreatedDate = now.AddDays(-25),
                OrderItems = new List<OrderItem>
                {
                    new OrderItem { ProductId = samsung.Id, Quantity = 1, UnitPrice = samsung.Price, LineTotal = samsung.Price * 1 }
                }
            };
            ...
            // Payments cho các order Shipped
            db.Payments.AddRange(
                new Payment
                {
                    ...
                    Status = "Paid",
                    Method = "BankTransfer",
                    CreatedDate = order1.CreatedDate.AddHours(2)
                },
                new Payment
                {
                    OrderId = order3.Id,
                    ...
                    Status = "Paid",
                    Method = "CreditCard",
                    CreatedDate = order3.CreatedDate.AddHours(1)
                }
            );
            db.SaveChanges();
        }
